...
// buildMappingTable returns a mapping table for si containing each of its
// supported message types.
func buildMappingTable(s Saga, si Instance) MappingTable {
	triggers, others := s.MessageTypes()
	types := triggers.Union(others)
	table := MappingTable{}

	for _, mt := range types.Members() {
		table[mt] = s.MapInstance(mt, si)
	}
