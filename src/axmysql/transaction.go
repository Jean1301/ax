...
// TransactionRepository is an implementation of ax.TransactionRepository that
// stores transaction data in a MySQL database.
type TransactionRepository struct {
	DB *sql.DB
}

// Begin starts or resumes a message transaction for the given message.
func (m *TransactionManager) Begin(ctx context.Context, id MessageID) (MessageTransaction, error) {
    
}

// Commit marks the message as handled and persists the messages in the
// transaction's outbox.
Commit(ctx context.Context, mtx MessageTransaction) error

// Finalize marks the transaction as complete, and clears the outbox.
Finalize(ctx context.Context, mtx MessageTransaction) error
